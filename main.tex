...
$\Psi\beta = U(V^\top\beta),$, where the left-hand side takes time $O(L^{|e|+1})$
and the right-hand side takes $O(L^{|e|}R + LR)$.
For PCFGs, we have $|e| = 2$, resulting in the term $V^\top\beta$ dominating the runtime.
This contrasts with HMMs, where both $V^\top\beta$ and the subsequent multiplication
by $U$ take the same amount of time.
