...
\title{Low-Rank Constraints for Fast Inference in Discrete Structured Models}
...
It returns the sum over latent structures, i.e. $p(x)$. Counting
...
These examples are instances of the hypergraph algorithm.
...
There are $T$ scoring matrices, $\Psi_t\in\R^{\mcL\times\mcL}$, with entries $[\Psi_{T-t}]_{z_t,z_{t+1}} = p(z_{t+1},x_t \mid z_t)$ corresponding to transitions. 
...
then the matrix-vector product can be computed in time
...
where $c_{z_u},c_v$ contain normalizing constants and clamped potentials (e.g. conditional probabilities), where we use $v = (z_1,z_2)$ for brevity.
...
they can be computed from unnormalized $\tilde{U}_e,\tilde{V}_e$ as follows: $c_{z_u} = [\tilde{U}_e\tilde{V}_e^\top\mathbf{1}]_{z_u}$ in time $O(L^{|e|}N)$, and similarly for $c_{z_v}$.
}
...
Validation perplexities on \textsc{Ptb} versus model scale, as well as speed in seconds per epoch.
}
\end{figure}
...
All our HMMs are outperformed by neural models, and also by VL-HMMs \citep{chiu2020scaling} which offer similar modeling advantages, shown in Tbl.~\ref{tbl:hmm}~(left).
...
very effective for language modeling, the VL-HMM relies on a hard clustering of states for emission. 
...
\section{Extension of the Low-Rank Constraints to Other Semirings}
Enforcing low-rank constraints in the scoring matrix $\Psi_e$
let to a speed increase for the key step in the hypergraph marginalization algorithm:
\begin{equation}
    \Psi_e\beta_v =  \left(U_e V_e^\top\right) \beta_v =  U_e\left(V_e^\top\beta_v\right),
\end{equation}
...
The key matrix-vector product step in scalar form is given by
\begin{align*}
...
while the second line takes $O(\mcL^{|e|}N)$ computation.
...
In the scalar form in the max-times semiring,
...
\end{align*}
preventing us from rewriting MAP hypergraph inference as a low-rank matrix-vector product.
